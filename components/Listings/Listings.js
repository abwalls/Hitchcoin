...
export default function Listings({ navigation }) {
  ...
  const listRideReqs = async () => {
    try {
      client
        .query({ query: gql(listRides) })
        .then(({ data: { listRides } }) => {
          console.log(listRides.items);
          setRequests(listRides.items);
        });
    } catch (err) {
      alert(err);
    }
      ...
      <KeyboardAvoidingView behaviour="padding" style={styles.signup_container}>
        ...
        >
          <View style={styles.signup_form_container}>
            <Text style={styles.signup_banner_text}>RIDE REQUESTS</Text>
          </View>
          <View style={styles.signup_form_container}>
            <TouchableOpacity onPress={listRideReqs}>
              <Text style={styles.login_button}>SHOW REQUESTS</Text>
            </TouchableOpacity>
            <View>
